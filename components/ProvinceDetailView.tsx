...
const ProvinceDetailView: React.FC<ProvinceDetailViewProps> = ({ province, onBack, onSelectAttraction }) => {
    ...
    <div className="animate-fade-in pb-32">
      <div className="relative w-full h-[40vh] md:h-[60vh] bg-slate-900 overflow-hidden shadow-2xl">
        ...
        <div className="absolute bottom-0 left-0 p-8 md:p-16 w-full">
          <span className="bg-yellow-500 text-slate-950 text-[10px] md:text-xs font-black px-4 md:px-5 py-1.5 md:py-2 rounded-xl tracking-tight mb-4 md:mb-6 inline-block shadow-2xl border-2 border-yellow-400/30">
            {t(`regions.${province.region}`)}
          </span>
          <h1 className="text-3xl md:text-8xl font-black text-white tracking-tighter leading-tight drop-shadow-2xl">{translatedProvinceName}</h1>
        </div>
      </div>

      <div className="container mx-auto px-6 md:px-10 mt-12 md:mt-16">
        <div className="max-w-4xl mb-16 md:mb-24">
          <h2 className="text-[10px] md:text-[11px] font-black text-slate-600 tracking-tight mb-4 md:mb-6">{t('detailView.about', { name: translatedProvinceName })}</h2>
          <p className="text-xl md:text-3xl text-slate-200 leading-relaxed font-black tracking-tight">
          ...
          <div className="mb-24">
            <div className="flex items-center gap-4 md:gap-6 mb-10 md:mb-12">
               ...
               <h2 className="text-2xl md:text-4xl font-black text-white tracking-tight">
                 {t('detailView.featuredAttractions', { name: translatedProvinceName })}
               </h2>
            </div>
